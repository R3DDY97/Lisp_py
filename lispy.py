...
def symbol_parser(lisp_str):
    ...
    if re_symbol:
        ...
        return re_symbol.group().strip(), lisp_str[re_symbol.end():]
    return None
...
#         if parg in LOCAL_ENV:
#             print(LOCAL_ENV[parg])
#             return
#         elif parg == 'define':
#             variable, value = parsed_lists[:2]
#         if isinstance(parg, list):
#             pargs.append(evaluator(parg))
#         else:
#             pargs.append(parg)
...
def main():
    try:
        os.system("clear||cls")
        while True:
            # evaluator(lisp_interpreter(repl()))
